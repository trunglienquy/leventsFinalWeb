...
<body>
    ...
    $product = mysqli_query($conn, "SELECT * FROM `allproduct` WHERE catagories = 4 ORDER BY 'idProduct' ASC LIMIT " . $itemOfPage . " OFFSET " . $offset . "");
    //create paging
    $totalProduct = mysqli_query($conn, "SELECT * FROM `allproduct` WHERE catagories = 4");
    $totalProduct = $totalProduct->num_rows;
    //ceil dùng để làm tròn số trang
    $totalPage = ceil($totalProduct / $itemOfPage);
    ?>

    <h2 class="name-intro">ÁO KHOÁC</h2>
